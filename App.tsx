...
const POSTPARTUM_TASKS = [
  { task: 'Periksa Luka Jahitan/SC', time: 'Pagi Hari', icon: <ShieldCheck size={16} /> },
  { task: 'Pijat Oksitosin (Lancar ASI)', time: 'Setiap Hari', icon: <Waves size={16} /> },
  { task: 'Konsumsi Sayuran Hijau', time: 'Sarapan/Maksi', icon: <Utensils size={16} /> }
];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    state.users.filter(u => u.role === UserRole.USER && !u.isPostpartum).forEach(patient => {
  ...
  const handleConfirmDeliverySubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!confirmingDelivery) return;
    const formData = new FormData(e.currentTarget);
    const data: DeliveryData = {
      deliveryDate: formData.get('deliveryDate') as string,
      deliveryType: formData.get('deliveryType') as any,
      maternalCondition: formData.get('maternalCondition') as any,
      babyCondition: formData.get('babyCondition') as any,
      babyGender: formData.get('babyGender') as any,
      babyWeight: parseFloat(formData.get('babyWeight') as string),
    };

    setState(prev => ({
      ...prev,
      users: prev.users.map(u => u.id === confirmingDelivery.id ? { ...u, isPostpartum: true, deliveryData: data } : u),
      alerts: prev.alerts.filter(a => a.patientId !== confirmingDelivery.id) // Clear missed alerts
    }));
    
    addLog('CONFIRM_DELIVERY', 'PATIENT', `Konfirmasi persalinan Ny. ${confirmingDelivery.name}. Selamat atas kelahirannya!`);
  ...
  const handleNewPregnancySubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!startingNewPregnancy) return;
    const formData = new FormData(e.currentTarget);
    const newHpht = formData.get('hpht') as string;
    ...
    setState(prev => ({
      ...
      users: prev.users.map(u => {
        if (u.id === startingNewPregnancy.id) {
          ...
          if (u.deliveryData) history.push(u.deliveryData);
          ...
          return {
            ...
            pregnancyNumber: u.pregnancyNumber + 1, // Increment Gravida
            selectedRiskFactors: [], // Reset risk for new assessment
            totalRiskScore: 0
          };
        }
        return u;
      }),
      // Clear old visits for active monitoring but keep in DB (logic could be filtered by date later)
      // ancVisits: prev.ancVisits // Visits remain in DB for overall history
    }));

    addLog('NEW_PREGNANCY', 'PATIENT', `Mendaftarkan kehamilan baru Ny. ${startingNewPregnancy.name}. G${startingNewPregnancy.pregnancyNumber + 1}`);
    setStartingNewPregnancy(null);
    showNotification(`Kehamilan baru ${startingNewPregnancy.name} berhasil didaftarkan.`);
  };
  ...
  const handleDeleteVisit = (visitId: string) => {
    ...
    setState(prev => ({ ...prev, ancVisits: prev.ancVisits.filter(v => v.id !== visitId) }));
  ...
  const getCurrentLocation = () => {
    ...
    if (!navigator.geolocation) { alert("Geolokasi tidak didukung."); setIsGettingLocation(false); return; }
    navigator.geolocation.getCurrentPosition(
      (pos) => { setFormCoords({ lat: pos.coords.latitude.toFixed(6), lng: pos.coords.longitude.toFixed(6) }); setIsGettingLocation(false); showNotification("Lokasi didapatkan"); },
  ...
  const DashboardHome = () => {
    ...
    if (currentUser?.role === UserRole.USER) {
      if (currentUser.isPostpartum) {
        // Dashboard Postpartum (NIFAS)
        const checklist = state.userChecklists[currentUser.id] || {};
        return (
          <div className="space-y-12 animate-in fade-in duration-700">
            <div className="bg-emerald-600 p-8 md:p-14 rounded-[3rem] md:rounded-[4.5rem] text-white shadow-2xl relative overflow-hidden group">
               <div className="relative z-10">
                 <h2 className="text-3xl md:text-5xl font-black tracking-tighter leading-none mb-6">Halo, Ibu Hebat! 🌸</h2>
                 <p className="text-emerald-100 font-bold text-sm md:text-lg max-w-lg mb-10 leading-relaxed">
                   Selamat atas kelahiran bayi {currentUser.deliveryData?.babyGender === 'L' ? 'Laki-laki' : 'Perempuan'}. Fokus saat ini adalah pemulihan Ibu dan tumbuh kembang Buah Hati.
                 </p>
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                    <div className="bg-white/10 backdrop-blur-md p-5 rounded-3xl border border-white/10">
                       <p className="text-[9px] font-black uppercase tracking-widest opacity-60">Tanggal Lahir</p>
                       <p className="text-xs font-black mt-1 leading-tight">{currentUser.deliveryData?.deliveryDate}</p>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md p-5 rounded-3xl border border-white/10">
                       <p className="text-[9px] font-black uppercase tracking-widest opacity-60">Berat Bayi</p>
                       <p className="text-2xl font-black mt-1">{currentUser.deliveryData?.babyWeight} <span className="text-xs">Kg</span></p>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md p-5 rounded-3xl border border-white/10">
                       <p className="text-[9px] font-black uppercase tracking-widest opacity-60">Metode</p>
                       <p className="text-xs font-black mt-2 leading-tight">{currentUser.deliveryData?.deliveryType}</p>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md p-5 rounded-3xl border border-white/10">
                       <p className="text-[9px] font-black uppercase tracking-widest opacity-60">Status Nifas</p>
                       <p className="text-xs font-black mt-2 leading-tight">PEMULIHAN</p>
                    </div>
                 </div>
               </div>
               <PartyPopper size={280} className="absolute -right-16 -bottom-16 opacity-10 rotate-12 pointer-events-none" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
              <div className="bg-white p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] shadow-sm border border-gray-100">
                 <div className="flex items-center justify-between mb-10">
                    <h3 className="text-xl md:text-2xl font-black text-gray-900 uppercase tracking-tighter flex items-center gap-4">
                       <ShieldCheck className="text-emerald-600" size={28} /> Rutinitas Nifas Sehat
                    </h3>
                 </div>
                 <div className="space-y-4">
                    {POSTPARTUM_TASKS.map((item, idx) => (
                      <button 
                         key={idx}
                         ...
                         className={`w-full flex items-center justify-between p-6 rounded-[2rem] border-4 transition-all group ${
                           checklist[item.task] ? 'bg-emerald-50 border-emerald-500' : 'bg-gray-50 border-transparent'
                         }`}
                      >
                         <div className="flex items-center gap-5">
                            <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-emerald-600 shadow-sm border border-gray-100">{item.icon}</div>
              ...
              <div className="bg-white p-10 rounded-[3rem] md:rounded-[4rem] shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
                 <div className="text-6xl md:text-8xl mb-6">👶</div>
                 <h3 className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em] mb-2">Pantauan Tumbuh Kembang</h3>
                 <p className="text-2xl md:text-3xl font-black text-gray-900 tracking-tighter">Jadwal Imunisasi Berikutnya</p>
                 <div className="mt-8 px-8 py-4 bg-emerald-50 text-emerald-600 rounded-full font-black text-xs uppercase tracking-widest">BCG & Polio 1 (Minggu Depan)</div>
              </div>
            </div>
          </div>
        );
        ...
        <div className="space-y-8 md:space-y-12 animate-in fade-in duration-700">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="space-y-8">
               ...
               <button onClick={() => handleNavigate('smart-card')} className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black uppercase text-[10px] tracking-widest shadow-xl flex items-center justify-center gap-4 hover:bg-slate-800 transition-all active:scale-95">
      ...
      return (
        <div className="space-y-10 animate-in fade-in duration-700">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
             ...
             <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col justify-between group hover:scale-[1.02] transition-transform">
                <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Ibu Melahirkan</p><h3 className="text-4xl font-black text-gray-900 mt-2 tracking-tighter">{state.users.filter(u => u.isPostpartum).length}</h3></div>
                <PartyPopper className="text-emerald-500 self-end" size={32} />
             </div>
    ...
    if (currentUser?.role === UserRole.NAKES) {
      ...
      }).filter(p => !p.isPostpartum && (p.risk.label === 'HITAM' || p.risk.label === 'MERAH'))
        ...
        <div className="space-y-10 animate-in fade-in duration-700">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             ...
             <div className="bg-white p-10 rounded-[3rem] border border-gray-100 flex flex-col items-center justify-center text-center">
                <PartyPopper size={48} className="text-emerald-100 mb-4" />
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6">{state.users.filter(u => u.isPostpartum).length} Ibu Graduasi</p>
                <button onClick={() => handleNavigate('patients')} className="w-full py-4 bg-emerald-50 text-emerald-600 rounded-xl text-[10px] font-black uppercase">Lihat Daftar</button>
             </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="p-4 md:p-8 lg:p-12 xl:p-16 max-w-[1600px] mx-auto">
             ...
             <div className="fixed inset-0 z-[100] bg-emerald-950/80 backdrop-blur-2xl flex items-start justify-center p-2 md:p-10 overflow-y-auto">
               <div className="bg-white w-full max-w-4xl rounded-[4.5rem] shadow-2xl my-auto animate-in zoom-in-95 duration-700 relative overflow-hidden">
                 <div className="bg-emerald-600 p-12 text-white flex justify-between items-center relative overflow-hidden">
                   <div className="relative z-10">
                     <h2 className="text-4xl font-black uppercase tracking-tighter">Konfirmasi Persalinan</h2>
                     <p className="text-emerald-100 font-bold text-xs uppercase tracking-widest mt-2">Ny. {confirmingDelivery.name}</p>
                   </div>
                   <button onClick={() => setConfirmingDelivery(null)} className="relative z-10 p-5 bg-white/10 hover:bg-white/20 rounded-2xl transition-all"><X size={18}/></button>
                   <PartyPopper size={200} className="absolute -right-10 -bottom-10 opacity-10" />
                 </div>
                 <form onSubmit={handleConfirmDeliverySubmit} className="p-16 space-y-12">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                     <div className="space-y-3">
                       <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">Tanggal Persalinan</label>
                       <input name="deliveryDate" type="date" className="w-full p-5 bg-gray-50 border-none rounded-2xl font-black" required />
                     </div>
                     <div className="space-y-3">
                       <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">Jenis Persalinan</label>
                       <select name="deliveryType" className="w-full p-5 bg-gray-50 border-none rounded-2xl font-black">
                         <option value="NORMAL">PERSALINAN NORMAL</option>
                         <option value="SC">OPERASI SESAR (SC)</option>
                         <option value="VAKUM">EKSTRAKSI VAKUM</option>
                         <option value="INDUKSI">INDUKSI PERSALINAN</option>
                       </select>
                     </div>
                   </div>
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                     <div className="space-y-3">
                       <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">Jenis Kelamin Bayi</label>
                       <select name="babyGender" className="w-full p-5 bg-gray-50 border-none rounded-2xl font-black">
                         <option value="L">LAKI-LAKI</option>
                         <option value="P">PEREMPUAN</option>
                       </select>
                     </div>
                     <div className="space-y-3">
                       <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">Berat Lahir (Kg)</label>
                       <input name="babyWeight" type="number" step="0.1" className="w-full p-5 bg-gray-50 border-none rounded-2xl font-black" required />
                     </div>
                     <div className="space-y-3">
                       <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">Kondisi Bayi</label>
                       <select name="babyCondition" className="w-full p-5 bg-gray-50 border-none rounded-2xl font-black">
                         <option value="SEHAT">SEHAT / BUGAR</option>
                         <option value="BBLR">BBLR (BERAT RENDAH)</option>
                   ...
                   <div className="space-y-3">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">Kondisi Ibu</label>
                      <select name="maternalCondition" className="w-full p-5 bg-gray-50 border-none rounded-2xl font-black">
                        <option value="SEHAT">SEHAT / PEMULIHAN NORMAL</option>
                        <option value="KOMPLIKASI">KOMPLIKASI PASCA SALIN</option>
                        <option value="MENINGGAL">MENINGGAL DUNIA</option>
                      </select>
                   </div>
                   <div className="flex gap-6">
                     <button type="submit" className="flex-1 py-7 bg-emerald-600 text-white rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-2xl">Simpan & Luluskan ANC</button>
                     <button type="button" onClick={() => setConfirmingDelivery(null)} className="px-10 py-7 bg-gray-100 text-gray-400 rounded-[2rem] font-black uppercase text-xs">Batal</button>
                   </div>
                 </form>
               </div>
             </div>
          ...
          {startingNewPregnancy && (
             <div className="fixed inset-0 z-[100] bg-indigo-950/80 backdrop-blur-2xl flex items-start justify-center p-2 md:p-10 overflow-y-auto">
               <div className="bg-white w-full max-w-xl rounded-[4.5rem] shadow-2xl my-auto animate-in zoom-in-95 duration-700 relative overflow-hidden">
                 <div className="bg-indigo-600 p-12 text-white flex justify-between items-center relative overflow-hidden">
                   <div className="relative z-10">
                     <h2 className="text-3xl font-black uppercase tracking-tighter">Daftar Kehamilan Baru</h2>
                     <p className="text-indigo-100 font-bold text-xs uppercase tracking-widest mt-2">Ny. {startingNewPregnancy.name} (G{startingNewPregnancy.pregnancyNumber + 1})</p>
                   </div>
                   <button onClick={() => setStartingNewPregnancy(null)} className="relative z-10 p-5 bg-white/10 hover:bg-white/20 rounded-2xl transition-all"><X size={18}/></button>
                   <Baby size={200} className="absolute -right-10 -bottom-10 opacity-10" />
                 </div>
                 <form onSubmit={handleNewPregnancySubmit} className="p-16 space-y-10">
                   <div className="space-y-3">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">HPHT Baru</label>
                      <input name="hpht" type="date" className="w-full p-5 bg-gray-50 border-none rounded-2xl font-black" required />
                      <p className="text-[9px] font-bold text-indigo-400 px-4 mt-2">Data persalinan sebelumnya akan diarsipkan dalam riwayat medis.</p>
                   </div>
                   <div className="flex gap-4">
                     <button type="submit" className="flex-1 py-7 bg-indigo-600 text-white rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-2xl">Mulai Siklus Baru</button>
                     <button type="button" onClick={() => setStartingNewPregnancy(null)} className="px-10 py-7 bg-gray-100 text-gray-400 rounded-[2rem] font-black uppercase text-xs">Batal</button>
                   </div>
                 </form>
               </div>
             </div>
